...
// HiddenFromMenu() (D-307) marks Blame (b) as Visible=false so git history
// remains discoverable in the ? full-screen overlay while the 12 most-used
// keys stay visible in the persistent menu.
// Matching by hidden.Help().Key (not index) is rename-safe for future phases.
